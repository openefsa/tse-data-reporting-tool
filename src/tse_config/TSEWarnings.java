...
public class TSEWarnings {
	...
	public static Message getSendMessageWarning(SendMessageException sendE, IDataset... reports) {
		...
		switch (type) {
		case NON_DP_USER:
			msg = Warnings.createFatal(TSEMessages.get("account.incomplete", PropertiesReader.getSupportEmail()),
					reports);
			break;
		...
		case USER_WRONG_ORG:
			...
			msg = Warnings.create(TSEMessages.get("error.title"), TSEMessages.get("account.wrong.org", orgCode),
					SWT.ICON_ERROR);
			break;
		case USER_WRONG_PROFILE:

			msg = Warnings.createFatal(TSEMessages.get("account.incorrect", PropertiesReader.getSupportEmail()),
					reports);

			break;

		default:
			// TODO check if it is possible to give a more excplicit message based on what
			// is returned from the DCF
			msg = Warnings.createFatal(
